...
def process_file(input_file, args, time_range, mass_range, custom_plot_size):
    ...
    try:
        spectra = list(mzxml.read(input_file))
    ...
    if len(tic_data) == 0:
        print(f"No valid MS1 spectra found in {input_file} for the specified ranges.")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Plot Total Ion Chromatograms (TICs) from mzXML files.")
    parser.add_argument('-input', help='Input mzXML file or directory', required=True)
    ...
    if os.path.isdir(args.input):
        mzxml_files = [os.path.join(args.input, f) for f in os.listdir(args.input) if f.lower().endswith('.mzxml')]
        if not mzxml_files:
            print(f"No mzXML files found in directory {args.input}.")
            return
        print(f"Detected {len(mzxml_files)} mzXML files in directory {args.input}.")
    elif os.path.isfile(args.input) and args.input.lower().endswith('.mzxml'):
        mzxml_files = [args.input]
        print(f"Processing single file: {args.input}")
    else:
        print(f"Invalid input path: {args.input}. Please specify a valid .mzXML file or directory.")
        return

    # Process each mzXML file
    for input_file in mzxml_files:
        process_file(input_file, args, time_range, mass_range, custom_plot_size)
