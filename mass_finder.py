...
def analyze_mass_spec(spectrum, mass_range, accuracy, element_dictionary, min_intensity):
    ...
    for index, mass in enumerate(mz_array):
        if mass_range[0] <= mass <= mass_range[1] and intensity_array[index] >= min_intensity:
            for delta_index, isotope_mass in enumerate(mz_array[index:]):
                if isotope_mass - mass > accuracy * 1.1:
                    break
                if abs(mass - isotope_mass) < accuracy * mass:
                    if element_dictionary is not None:
                        ...
                        if formulas:
                            isotope_masses.append({'index': index,
                                                   ...
                                                   'isotope_intensity': intensity_array[index + delta_index],
                                                   'mass': round(mass, 4),
    ...
    return (float(spectrum.get('retentionTime', 0)), isotope_masses) if len(isotope_masses) > 0 else None
...
def main():
    ...
    for file in files:
        ...
        analyzed_spectra = pool.starmap(analyze_mass_spec, [(data.get_by_index(int(index) - 1), mass_range,
                                                             args.accuracy, element_dictionary, args.min_intensity)
