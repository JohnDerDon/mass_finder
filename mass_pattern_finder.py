    ...
    if len(elements) > 1:
        ...
        for n_element in range(_min, _max + 1):
            ...
            if n_element * mass_element > mass + absolute_accuracy:
                break
            elif abs(n_element * mass_element - mass - sum([element[0] * element[2] for element in
                                                            truncated_element_dictionary.values()])) < absolute_accuracy:  # This present a bug still. It will not take the min and max of other elements into account
                formula_list.append(f"{element}{n_element}")
            else:
                subformulas = find_coefficients_in_mass(truncated_element_dictionary, mass - n_element * mass_element,
                                                        absolute_accuracy)
                formula_list.extend(
                    [f"{element}{n_element}_{subformula}" for subformula in subformulas if 'None' not in subformula])
        ...
        if (mass - absolute_accuracy) / mass_element < round(mass / mass_element, 0) < (
                mass + absolute_accuracy) / mass_element:
...
def analyze_mass_spec(spectrum, mass_difference, difference_intensity, min_intensity, element_dictionary, accuracy,
                      mass_range):
    ...
    for index, mass in enumerate(spectrum['m/z array']):
        ...
        for delta_index, isotope_mass in enumerate(spectrum['m/z array'][index:]):
            ...
            if abs(mass + mass_difference - isotope_mass) < accuracy * mass and \
                    difference_intensity[0] < abs(
                spectrum['intensity array'][index + delta_index] / spectrum['intensity array'][index]) < \
...
def construct_element_dictionary(element_string):
    ...
    return {element.split('-')[1]: [int(element.split('-')[0]), int(element.split('-')[2]), find_atomic_mass(element)]
            for element in elements}
...
def main():
    ...
    parser.add_argument('-mass_difference', help='Mass difference to search. Default is chlorine', type=float,
                        default=1.99704989)
    ...
    parser.add_argument('-intensity_diff',
                        help='Min and max intensity ratios of isotope peaks to be detected. Default is for chlorination: "0.25-1".',
                        default='0.25-1', type=str)
    ...
    for file in files:
        ...
        analyzed_spectra = pool.starmap(analyze_mass_spec, [(data.get_by_index(int(index) - 1), args.mass_difference,
                                                             difference_intensities, args.min_intensity,
                                                             element_dictionary, args.accuracy, mass_range)
        ...
        with open(os.path.splitext(file)[0] + '_analyzed.txt', 'w') as output_file:
            ...
            for retention_time, spectrum in analyzed_spectra:
                ...
                for peak in spectrum:
                    output_file.write(
                        f"\tMass: {peak['mass']}\tIntensity: {peak['intensity']}{nl}{nl.join([clean_formula(formula) for formula in peak['formulas']])}\n")
        if args.full_range:
