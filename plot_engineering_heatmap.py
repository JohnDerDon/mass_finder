...
def plot_heatmap(mean_csv, sum_intensity_csv, std_dev_csv, data_qual_csv):
    ...
    fig, (ax_legend, ax) = plt.subplots(1, 2, gridspec_kw={'width_ratios': [1, 10]}, figsize=(14, 10))
    ...
    ax_legend.axis('off')
    ax_legend.text(0.5, 0.94, "standard\ndeviation", fontsize=20, ha='center')
    ...
    legend_y_positions = [0.88, 0.74, 0.60]
    ...
    for i, std_dev_value in enumerate(example_std_dev_values):
        ...
        ax_legend.text(0.5, legend_y_positions[i]-0.08, f"{std_dev_value:.2f}", fontsize=20, ha='center')

    # Legend for circle intensity sizes
    ax_legend.axis([0, 1, 0, 1])
    ax_legend.axis('off')
    ax_legend.text(0.5, 0.38, "overall\nsum\nintensity", fontsize=20, ha='center')

    example_sum_values = [10**9, 10**7, 10**5]  # example values to cover log range
    normalized_example = np.log10(example_sum_values) / 10.0
    legend_y_positions = [0.31, 0.17, 0.04]
    ...
    for i, (norm_val, sum_val) in enumerate(zip(normalized_example, example_sum_values)):
        ...
        ax_legend.text(0.5, legend_y_positions[i]-(0.085*(9-i)/9), f"$10^{norm_val*10:.0f}$".replace("+0", ""), fontsize=20, ha='center')
